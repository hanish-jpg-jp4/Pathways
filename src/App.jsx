...
function mLevel(s){if(s>=85)return{label:"Excellent Match",color:"#4ade80"};if(s>=72)return{label:"Strong Match",color:"#FF6B35"};if(s>=55)return{label:"Possible Match",color:"#FFE66D"};return{label:"Explore",color:"#aaa"};}
...
function Btn({children,onClick,variant="primary",style={}}){
  ...
  const styles={
    ...
    secondary:{...base,background:"transparent",color:"#aaa",border:"1px solid #222",transform:hov?"translateY(-1px)":"translateY(0)",color:hov?"#fff":"#aaa",borderColor:hov?"#aaa":"#222"},
    ...
    <div style={{maxWidth:640,margin:"0 auto",padding:"0 20px 60px"}}>
      ...
      <div style={{marginBottom:40}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
          ...
          <span style={{color:"#aaa",fontSize:13,fontWeight:500}}>{prog}%</span>
        </div>
        ...
        <p style={{color:"#999",fontSize:13,marginTop:10}}>{SECTION_DESCS[sec]}</p>
      </div>
      ...
      <div style={{opacity:vis?1:0,transform:vis?"translateY(0)":"translateY(12px)",transition:"all 0.22s ease"}}>
        <div style={{background:"#0e0e0e",border:"1px solid #1a1a1a",borderRadius:24,padding:"40px 36px",marginBottom:20,boxShadow:"0 20px 60px #00000060"}}>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:20}}>
            ...
            <span style={{color:"#999",fontSize:13}}>of {sQs[sec].length}</span>
          </div>
        ...
        {(sec>0||qi>0)&&(
          <button onClick={back} style={{background:"transparent",color:"#aaa",border:"none",fontSize:13,cursor:"pointer",padding:"8px 0",display:"flex",alignItems:"center",gap:6}}
            onMouseEnter={e=>e.currentTarget.style.color="#aaa"} onMouseLeave={e=>e.currentTarget.style.color="#aaa"}>
            ← Back
          </button>
        )}
      </div>
  ...
  return(
    <div style={{position:"relative",width:size,height:size,flexShrink:0}}>
      ...
      <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column"}}>
        ...
        <span style={{fontSize:size*0.1,color:"#aaa"}}>/100</span>
      </div>
    </div>
  );
}
  ...
  return(
    <div ref={ref} style={{maxWidth:800,margin:"0 auto",opacity:inView?1:0,transform:inView?"translateY(0)":"translateY(30px)",transition:"all 0.8s ease",padding:"0 20px"}}>
      ...
      <div style={{textAlign:"center",marginBottom:36}}>
        ...
        <p style={{color:"#aaa",fontSize:15}}>Top values: {result.topValues.join(" · ")}</p>
      </div>
      ...
      <div style={{background:"#0e0e0e",border:"1px solid #1a1a1a",borderRadius:20,padding:"24px 28px",marginBottom:16,boxShadow:"0 8px 40px #00000040"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:18}}>
          ...
          <span style={{color:"#999",fontSize:12}}>Based on your answers</span>
        </div>
        {Object.entries(result.riasec).map(([c,s])=>(
          <div key={c} style={{display:"flex",alignItems:"center",gap:14,marginBottom:11}}>
            <span style={{color:"#aaa",fontSize:13,width:110,flexShrink:0}}>{rl[c]}</span>
            <div style={{flex:1,height:4,background:"#141414",borderRadius:999,overflow:"hidden"}}>
              <div style={{height:"100%",background:s>=70?"#FF6B35":s>=50?"#FF6B3580":"#999",borderRadius:999,width:`${s}%`,transition:"width 1.2s cubic-bezier(.4,0,.2,1)"}}/>
            </div>
            <span style={{color:"#aaa",fontSize:13,width:30,textAlign:"right",fontWeight:600}}>{s}</span>
          </div>
        ))}
      </div>
      ...
      <div style={{display:"flex",gap:6,marginBottom:16,background:"#0a0a0a",border:"1px solid #141414",borderRadius:16,padding:6}}>
        {[["careers","🎯 Careers"],["opportunities","🚀 Opportunities"],["roadmap","🗺️ Roadmap"]].map(([t,l])=>(
          <button key={t} onClick={()=>setTab(t)} style={{flex:1,background:tab===t?"#FF6B35":"transparent",color:tab===t?"#fff":"#aaa",border:"none",borderRadius:12,padding:"10px 0",fontSize:13,fontWeight:600,cursor:"pointer",transition:"all 0.2s"}}>{l}</button>
        ))}
      </div>
          ...
          {result.careerMatches.slice(0,6).map((c,i)=>(
            <div key={c.id} onMouseEnter={()=>setHovCard(i)} onMouseLeave={()=>setHovCard(null)}
              ...
              <div style={{flex:1,minWidth:0}}>
                ...
                <p style={{color:"#aaa",fontSize:13,lineHeight:1.6,marginBottom:10}}>{c.description}</p>
                <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:10}}>
                  {c.fields.map(f=><span key={f} style={{background:"#141414",border:"1px solid #1e1e1e",color:"#aaa",fontSize:11,borderRadius:999,padding:"3px 10px"}}>{f}</span>)}
                  {c.tags?.slice(0,2).map(t=><span key={t} style={{background:"#141414",border:"1px solid #1e1e1e",color:"#aaa",fontSize:11,borderRadius:999,padding:"3px 10px"}}>#{t}</span>)}
                </div>
                {c.explorationActivities&&<p style={{color:"#FF6B35",fontSize:13}}><span style={{color:"#aaa"}}>Try: </span>{c.explorationActivities[0]}</p>}
              </div>
            </div>
          ))}
        </div>
          ...
          {result.opportunities.map(o=>(
            <div key={o.id} style={{background:"#0e0e0e",border:"1px solid #1a1a1a",borderRadius:16,padding:"20px 22px",display:"flex",gap:16,alignItems:"flex-start"}}>
              <div style={{flexShrink:0,background:"#FF6B3515",border:"1px solid #FF6B3530",borderRadius:12,padding:"8px 12px",textAlign:"center",minWidth:52}}>
                ...
                <div style={{color:"#aaa",fontSize:10}}>match</div>
              </div>
              <div style={{flex:1}}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4,flexWrap:"wrap"}}>
                  <span style={{fontWeight:600,fontSize:15,color:"#f0f0f0"}}>{o.name}</span>
                  <span style={{background:"#141414",border:"1px solid #222",color:"#aaa",fontSize:11,borderRadius:999,padding:"2px 8px"}}>{o.type}</span>
                  <span style={{background:"#141414",border:"1px solid #222",color:"#aaa",fontSize:11,borderRadius:999,padding:"2px 8px"}}>{o.difficulty}</span>
                </div>
                <p style={{color:"#aaa",fontSize:12,marginBottom:4}}>{o.organization}</p>
                <p style={{color:"#aaa",fontSize:13,lineHeight:1.5,marginBottom:8}}>{o.description}</p>
                {o.url?<a href={o.url} target="_blank" rel="noopener noreferrer" style={{color:"#FF6B35",fontSize:13,textDecoration:"none",fontWeight:500}}>Learn more →</a>:<span style={{color:"#999",fontSize:13}}>Search online for current openings</span>}
              </div>
            </div>
          ))}
        </div>
      ...
      <div style={{padding:"20px 5% 16px",borderBottom:"1px solid #0e0e0e",flexShrink:0}}>
        <div style={{maxWidth:720,margin:"0 auto",display:"flex",alignItems:"center",gap:14}}>
          ...
          <div>
            ...
            <div style={{color:"#aaa",fontSize:13}}>Ask about careers, internships, or your next step.</div>
          </div>
          <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:6}}>
            <div style={{width:7,height:7,borderRadius:"50%",background:"#4ade80",animation:"pulse-slow 2s infinite"}}/>
            <span style={{color:"#aaa",fontSize:12}}>Online</span>
          </div>
        </div>
      </div>
      ...
      <div style={{padding:"14px 5% 22px",borderTop:"1px solid #0e0e0e",flexShrink:0}}>
        <div style={{maxWidth:720,margin:"0 auto",display:"flex",gap:10,alignItems:"flex-end"}}>
          ...
          <button onClick={send} disabled={!inp.trim()||load}
            style={{background:inp.trim()&&!load?"#FF6B35":"#111",color:inp.trim()&&!load?"#fff":"#999",border:"none",borderRadius:12,width:46,height:46,fontSize:16,cursor:inp.trim()&&!load?"pointer":"default",transition:"all 0.2s",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",boxShadow:inp.trim()&&!load?"0 4px 16px #FF6B3540":"none"}}>→</button>
        </div>
  ...
  return(
    <div style={{background:"#080808",minHeight:"100vh",color:"#fff"}}>
      ...
      <section style={{padding:"80px 5% 60px",maxWidth:860,margin:"0 auto"}}>
        ...
        <p style={{color:"#aaa",fontSize:18,lineHeight:1.8,maxWidth:560,fontWeight:300}}>We got tired of watching our peers pick careers based on salary charts and parental pressure — with no real sense of what their future actually looked like day-to-day. So we built Pathways.</p>
      </section>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:40}}>
          {[{year:"2023",title:"The Problem",desc:"Students were picking majors based on salary charts and parental pressure — with no real sense of what the day-to-day looked like.",icon:"🔍"},{year:"2024",title:"The Idea",desc:"A simple quiz turned into a full platform — matching interests to careers, internships, and real field experiences.",icon:"💡"},{year:"2025",title:"First Students",desc:"We launched a beta with 200 students across 12 schools. 97% said they found clearer direction within a single session.",icon:"🚀"},{year:"2026",title:"Today",desc:"Pathways is growing. We're expanding our career database, refining our AI counselor, and partnering with schools nationwide.",icon:"🌍"}].map(item=>(
            <div key={item.year} style={{background:"#0e0e0e",border:"1px solid #161616",borderRadius:20,padding:"28px 24px"}}>
              ...
              <p style={{color:"#aaa",fontSize:14,lineHeight:1.7}}>{item.desc}</p>
            </div>
          ))}
        </div>
          ...
          <div style={{display:"flex",gap:14}}>
            {[{name:"Hanish Dudam",role:"Founder"},{name:"Sriman Padamatinti",role:"Co-Founder"}].map(p=>(
              <div key={p.role} style={{flex:1,background:"#0e0e0e",border:"1px solid #161616",borderRadius:16,padding:"22px 18px",textAlign:"center"}}>
                ...
                <div style={{color:"#aaa",fontSize:13}}>{p.role}</div>
              </div>
            ))}
          </div>
        </div>
    ...
    <div style={{background:"#080808",minHeight:"100vh",color:"#fff"}}>
      ...
      <div style={{maxWidth:860,margin:"0 auto",padding:"48px 5%"}}>
        ...
        <div style={{marginBottom:40}}>
          ...
          <p style={{color:"#aaa",fontSize:15}}>Here are your past career discovery results.</p>
        </div>

        {loading&&(
          <div style={{textAlign:"center",padding:"60px 0",color:"#999",fontSize:15}}>Loading your results...</div>
        )}
        ...
        {!loading&&results.length===0&&(
          <div style={{background:"#0e0e0e",border:"1px solid #141414",borderRadius:24,padding:"60px 40px",textAlign:"center"}}>
            ...
            <p style={{color:"#aaa",fontSize:15,marginBottom:28}}>Take the career quiz to discover your personality type and top career matches.</p>
                  ...
                  <button onClick={()=>setSelected(isOpen?null:i)} style={{width:"100%",background:"none",border:"none",cursor:"pointer",padding:"22px 24px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:16,textAlign:"left"}}>
                    <div style={{display:"flex",alignItems:"center",gap:16,flex:1,minWidth:0}}>
                      ...
                      <div style={{minWidth:0}}>
                        ...
                        <div style={{color:"#aaa",fontSize:13}}>{date} · {careers.length} career matches</div>
                      </div>
                    </div>
                    <div style={{display:"flex",gap:8,alignItems:"center",flexShrink:0}}>
                      {careers.slice(0,2).map(c=><span key={c.name||c.id} style={{background:"#141414",border:"1px solid #1e1e1e",color:"#aaa",fontSize:11,borderRadius:999,padding:"3px 10px",display:"none"}}>{c.name}</span>)}
                      <span style={{color:"#aaa",fontSize:18,transition:"transform 0.3s",transform:isOpen?"rotate(180deg)":"rotate(0deg)"}}>⌄</span>
                    </div>
                  </button>
                        ...
                        <div style={{marginBottom:20}}>
                          ...
                          {Object.entries(riasec).map(([c,s])=>(
                            <div key={c} style={{display:"flex",alignItems:"center",gap:12,marginBottom:8}}>
                              <span style={{color:"#aaa",fontSize:12,width:90,flexShrink:0}}>{rl[c]||c}</span>
                              ...
                              <span style={{color:"#999",fontSize:12,width:28,textAlign:"right"}}>{s}</span>
                            </div>
                          ))}
                        </div>
                      )}
                              ...
                              return(
                                <div key={j} style={{background:"#141414",border:"1px solid #1a1a1a",borderRadius:14,padding:"16px 18px",display:"flex",gap:14,alignItems:"flex-start"}}>
                                  <div style={{flexShrink:0,textAlign:"center",minWidth:52}}>
                                    ...
                                    <div style={{color:"#999",fontSize:10}}>/100</div>
                                  </div>
                                  <div style={{flex:1}}>
                                    <div style={{fontWeight:600,fontSize:15,marginBottom:3}}>{c.name}</div>
                                    {fullCareer&&<p style={{color:"#aaa",fontSize:13,lineHeight:1.5,marginBottom:6}}>{fullCareer.description}</p>}
                                    {fullCareer&&<div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                                      {fullCareer.fields.map(f=><span key={f} style={{background:"#1a1a1a",border:"1px solid #222",color:"#aaa",fontSize:11,borderRadius:999,padding:"2px 8px"}}>{f}</span>)}
                                    </div>}
                                  </div>
                                </div>
                              );
    ...
    <div onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
        <div>
          ...
          <div style={{color:"#aaa",fontSize:12}}>{career.fields[0]}</div>
        </div>
        <span style={{color:"#FF6B35",fontSize:18,transition:"transform 0.3s",transform:hov?"translateX(3px)":"translateX(0)"}}>→</span>
      </div>
      <p style={{color:"#aaa",fontSize:13,lineHeight:1.6,marginBottom:14}}>{career.description}</p>
      <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
        {career.skills?.slice(0,3).map(s=><span key={s} style={{background:"#141414",border:"1px solid #1e1e1e",color:"#aaa",fontSize:11,borderRadius:999,padding:"3px 10px"}}>{s}</span>)}
      </div>
      {hov&&<div style={{marginTop:14,paddingTop:14,borderTop:"1px solid #1a1a1a"}}>
        ...
        <p style={{color:"#aaa",fontSize:12,lineHeight:1.5}}>{career.explorationActivities[0]}</p>
      </div>}
    </div>
  );
}
        ...
        <div style={{display:"flex",gap:28,alignItems:"center"}}>
          {[["How It Works","howitworks"],["Our Story","ourstory"],["Want Help?","talktous"]].map(([l,t])=>(
            <a key={l} href="#" onClick={e=>{e.preventDefault();t==="ourstory"?setPage("ourstory"):t==="talktous"?setPage("talktous"):scrollTo(t);}}
              style={{color:"#aaa",fontSize:14,fontWeight:500,textDecoration:"none",transition:"color 0.2s"}}
              onMouseEnter={e=>e.currentTarget.style.color="#fff"} onMouseLeave={e=>e.currentTarget.style.color="#aaa"}>{l}</a>
          ))}
          {isSignedIn?(
            <div style={{display:"flex",gap:10,alignItems:"center"}}>
              <Btn variant="ghost" onClick={()=>setPage("dashboard")} style={{padding:"8px 16px",fontSize:13,color:"#aaa"}}>My Results</Btn>
              <UserButton afterSignOutUrl="#" appearance={{elements:{avatarBox:{width:34,height:34}}}}/>
            </div>
          ):(
      ...
      <section style={{minHeight:"100vh",display:"flex",alignItems:"center",padding:"100px 5% 60px",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{display:"flex",alignItems:"center",width:"100%",gap:60,maxWidth:1200,margin:"0 auto"}}>
          ...
          <div style={{flex:"0 0 55%",maxWidth:"55%"}} ref={heroRef}>
            ...
            <p style={{color:"#aaa",fontSize:"clamp(15px,1.8vw,18px)",lineHeight:1.75,maxWidth:480,marginBottom:36,fontWeight:400,opacity:heroInView?1:0,transform:heroInView?"translateY(0)":"translateY(20px)",transition:"all 0.7s ease 0.3s"}}>
            ...
            <div style={{display:"flex",gap:16,marginTop:40,flexWrap:"wrap",opacity:heroInView?1:0,transition:"all 0.7s ease 0.5s"}}>
              {["Not a personality test.","Not a list of random careers.","Built around you."].map(t=>(
                <div key={t} style={{display:"flex",alignItems:"center",gap:6}}>
                  ...
                  <span style={{color:"#aaa",fontSize:13,fontWeight:500}}>{t}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{maxWidth:1200,margin:"0 auto",display:"flex",gap:0}}>
            {[{n:"25+",l:"Career Paths"},{n:"5",l:"Discovery Sections"},{n:"15+",l:"Opportunities Matched"}].map((s,i)=>(
              <div key={s.l} style={{flex:1,padding:"20px 0",paddingRight:40,borderRight:i<2?"1px solid #0e0e0e":"none",paddingLeft:i>0?40:0}}>
                ...
                <div style={{color:"#999",fontSize:13,marginTop:2,fontWeight:500}}>{s.l}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="howitworks" style={{padding:"100px 5%",borderTop:"1px solid #0e0e0e"}}>
        <div ref={howRef} style={{maxWidth:1100,margin:"0 auto"}}>
          <div style={{textAlign:"center",marginBottom:64,opacity:howInView?1:0,transform:howInView?"translateY(0)":"translateY(24px)",transition:"all 0.8s ease"}}>
            ...
            <p style={{color:"#aaa",fontSize:16,maxWidth:500,margin:"0 auto",lineHeight:1.7}}>Pathways considers your interests, experience, work style, and what you're actually willing to try.</p>
          </div>
            ...
            {[
              ...
              <div key={s.n} style={{background:"#0a0a0a",border:"1px solid #111",borderRadius:20,padding:"32px 28px",opacity:howInView?1:0,transform:howInView?"translateY(0)":"translateY(30px)",transition:`all 0.7s ease ${i*0.12}s`}}>
                <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:16}}>
                  ...
                  <span style={{color:"#999",fontSize:12,fontWeight:700,letterSpacing:1}}>{s.n}</span>
                </div>
                <h3 style={{fontWeight:700,fontSize:22,marginBottom:10,letterSpacing:-0.5}}>{s.title}</h3>
                <p style={{color:"#aaa",fontSize:14,lineHeight:1.7}}>{s.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{padding:"80px 5%",background:"#050505"}}>
        <div style={{maxWidth:1100,margin:"0 auto",display:"flex",gap:60,alignItems:"center",flexWrap:"wrap"}}>
          <div style={{flex:"0 0 40%",minWidth:280}}>
            ...
            <p style={{color:"#aaa",fontSize:15,lineHeight:1.7}}>Most career quizzes stop at interests. Pathways goes further — matching you based on how you actually want to work.</p>
          </div>
            ...
            {[["🧠","Interests","What you genuinely enjoy"],["💼","Experience","What you've actually done"],["⚙️","Work Style","How you like to work"],["🎯","Preferences","What matters in a career"],["⚡","Reality","What you can handle day-to-day"],["🔬","Exploration","What you'd be willing to try"]].map(([icon,label,desc])=>(
              <div key={label} style={{background:"#0e0e0e",border:"1px solid #141414",borderRadius:16,padding:"18px 16px",display:"flex",gap:12,alignItems:"flex-start"}}>
                ...
                <div>
                  ...
                  <div style={{color:"#aaa",fontSize:12,lineHeight:1.5}}>{desc}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <section style={{padding:"100px 5%"}}>
        <div style={{maxWidth:1100,margin:"0 auto"}} ref={prevRef}>
          <div style={{textAlign:"center",marginBottom:52,opacity:prevInView?1:0,transform:prevInView?"translateY(0)":"translateY(24px)",transition:"all 0.8s ease"}}>
            ...
            <p style={{color:"#aaa",fontSize:15,maxWidth:480,margin:"0 auto",lineHeight:1.7}}>Here's a preview of the career cards you'll receive. Take the quiz to see which ones match you.</p>
          </div>
        ...
        {step==="intro"&&(
          <div ref={ctaRef} style={{maxWidth:700,margin:"0 auto",textAlign:"center",opacity:ctaInView?1:0,transform:ctaInView?"translateY(0)":"translateY(24px)",transition:"all 0.8s ease"}}>
            <div style={{background:"radial-gradient(ellipse at center,rgba(255,107,53,0.08) 0%,transparent 70%)",borderRadius:32,padding:"60px 40px",border:"1px solid #111",boxShadow:"0 40px 80px #00000060"}}>
              ...
              <p style={{color:"#aaa",fontSize:16,lineHeight:1.7,maxWidth:480,margin:"0 auto 36px"}}>You don't need to know exactly what you want to do yet. You just need to start exploring.</p>
              <div style={{display:"flex",gap:10,justifyContent:"center",flexWrap:"wrap",marginBottom:36}}>
                {[["📋","57 Questions"],["⏱️","~8 Minutes"],["🎯","25+ Careers"],["🚀","Instant Results"]].map(([i,l])=>(
                  <div key={l} style={{background:"#0e0e0e",border:"1px solid #141414",borderRadius:999,padding:"8px 16px",display:"flex",alignItems:"center",gap:6}}>
                    <span style={{fontSize:14}}>{i}</span><span style={{color:"#aaa",fontSize:13,fontWeight:500}}>{l}</span>
                  </div>
                ))}
              </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:32}}>
            {[{l:"Pick classes for friends, not fit",i:"👥"},{l:"Choose majors for salary alone",i:"💰"},{l:"No idea what internships exist",i:"🔍"},{l:"Never lived a day in their future field",i:"📆"},{l:"Doing work, but losing curiosity",i:"📉"},{l:"No one sees the middle group",i:"🫥"}].map(p=>(
              <div key={p.l} style={{background:"#0a0a0a",border:"1px solid #0e0e0e",borderRadius:14,padding:"18px 20px",display:"flex",alignItems:"center",gap:14}}>
                <span style={{fontSize:20}}>{p.i}</span>                <span style={{color:"#aaa",fontSize:14,lineHeight:1.5}}>{p.l}</span>
              </div>
            ))}
          </div>
        ...
        <div style={{maxWidth:1100,margin:"0 auto"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:40,flexWrap:"wrap",gap:32}}>
            <div style={{maxWidth:280}}>
              <Logo/>
              <p style={{color:"#999",fontSize:14,lineHeight:1.7,marginTop:14}}>Career discovery built for your generation.</p>
            </div>
            <div style={{display:"flex",gap:48,flexWrap:"wrap"}}>
              <div>
                <div style={{color:"#aaa",fontSize:12,fontWeight:600,letterSpacing:1,textTransform:"uppercase",marginBottom:16}}>Navigate</div>
                {[["How It Works",()=>scrollTo("howitworks")],["Our Story",()=>setPage("ourstory")],["Find My Path",()=>scrollTo("quiz")],["Talk to Aria",()=>setPage("talktous")]].map(([l,fn])=>(
                  <div key={l} style={{marginBottom:10}}>
                    <a href="#" onClick={e=>{e.preventDefault();fn();}} style={{color:"#999",fontSize:14,textDecoration:"none",transition:"color 0.2s"}}
                      onMouseEnter={e=>e.currentTarget.style.color="#fff"} onMouseLeave={e=>e.currentTarget.style.color="#999"}>{l}</a>
                  </div>
                ))}
              </div>
              <div>
                <div style={{color:"#aaa",fontSize:12,fontWeight:600,letterSpacing:1,textTransform:"uppercase",marginBottom:16}}>Legal</div>
                {[["Privacy Policy","#"],["Terms of Use","#"]].map(([l,h])=>(
                  <div key={l} style={{marginBottom:10}}>
                    <a href={h} style={{color:"#999",fontSize:14,textDecoration:"none"}}
                      onMouseEnter={e=>e.currentTarget.style.color="#fff"} onMouseLeave={e=>e.currentTarget.style.color="#999"}>{l}</a>
                  </div>
                ))}
              </div>
